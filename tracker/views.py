...
def transactions_liste(request):
    ...
    transactions_page =  paginator.page(1) #default to 1 when this view is triggered
...
def get_transactions(request):
    page = request.GET.get('page', 1) # If ?page=2
    transactions_filter = TransactionFilter(
        request.GET,  
        queryset=Transaction.objects.filter(user=request.user).select_related('category')
    )
...
def transaction_charts(request):
    ...
    income_expenses_bar = plot_income_expenses_bar_chart(transactions_filter.qs)
    ...
    context = {
        'filter': transactions_filter,
        'income_expense_barchart': income_expenses_bar.to_html(),
...
def import_transactions(request):
    if request.method == 'POST':
        ...
        result = resouce.import_data(dataset, user=request.user, dry_run=True)
        ...
        if not result.has_errors():
            resouce.import_data(dataset, user=request.user, dry_run=False)
            context = {'message': f'{len(dataset)} Transactions were uploaded successfully!'}
        else:
            context = {'message': 'Transactions were not uploaded!'}
            return render(request, 'tracker/partials/transaction-success.html', context)
