...
def test_transaction_type_filter(user_transactions, client):
    ...
    respose = client.get(reverse('transactions-list'), GET_params)

    qs = respose.context['filter'].qs
...
def test_category_filter(user_transactions, client):
    ...
    # Get first two categories PKs(Primary Keys)
...
@pytest.mark.django_db
def transaction_dict_params(user, transaction_dict_params, client):
        client.force_login(user)
